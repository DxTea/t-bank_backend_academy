from .custom_errors import InvalidMenuChoiceError, InvalidUserGuessError, \
    HintUsageError
...
class InputHandler:
    ...
    def get_user_input(prompt: str) -> str:
        ...
        Args:
            prompt (str): Строка, которую нужно вывести перед User input.
    ...
    def validate_menu_choice(choice: str, valid_choices: list) -> str:
        ...
        if choice not in valid_choices:
            raise InvalidMenuChoiceError(
                f"Недопустимый ввод: '{choice}'. "
                f"Пожалуйста, введите номер пункта меню.")
        return choice
    ...
    def validate_user_guess(guess: str) -> str:
        ...
        """
        if guess not in ["1", "2", "3", "4"] and (
                not guess.isalpha() or len(guess) > 1):
            raise InvalidUserGuessError(
                f"Недопустимый ввод: '{guess}'. "
                f"Пожалуйста, введите номер пункта меню или букву.")
        return guess
    ...
    def validate_hint_usage(hint_index: int, hints: list) -> None:
        ...
        if hints[hint_index].startswith("Подсказка"):
            raise HintUsageError(
                f"Недопустимый ввод: '{hint_index + 1}'. "
                f"Подсказка {hint_index + 1} уже использована.")

...
from src.custom_errors import (InvalidMenuChoiceError, HintUsageError,
                               InvalidUserGuessError,
...
class GameLoop:
    ...
    def handle_hint(self, guess: str, hints: List[str],
        ...
        try:
            ...
            hints[
                hint_index] = (f"Подсказка {guess}: "
                               f"{game.current_word.hints[hint_index]}")
    ...
    def handle_main_menu_choice(self, choice):
        ...
        """
        choice = InputHandler.validate_menu_choice(choice,
                                                   ["1", "2", "3", "4"])
        match choice:
            case "1":
                self.start_game()
    ...
    def choose_category(self) -> None:
        ...
        """
        self.choose_option("категории", self.word_dictionary.unique_categories,
                           "category")
    ...
    def choose_difficulty(self) -> None:
        ...
        """
        self.choose_option("сложности", ["Легко", "Средне", "Сложно"],
                           "difficulty")

    def choose_option(self, option_name: str, options: List[str],
        ...
        while True:
            ...
            try:
                option_choice = InputHandler.get_user_input(
                    f"Введите номер {option_name}: ")
                InputHandler.validate_menu_choice(option_choice,
                                                  [str(i) for i in
                                                   range(len(options) + 1)])
                if option_choice == "0":
    ...
    def set_random_category_and_difficulty_if_needed(self) -> None:
        """
        Метод для установки случайной категории и сложности,
        если это необходимо.
        """
        if self.category == "Случайно":
            self.category = random.choice(
                self.word_dictionary.unique_categories)
        if self.difficulty == "Случайно":
            self.difficulty = random.choice(
                self.word_dictionary.unique_difficulties)

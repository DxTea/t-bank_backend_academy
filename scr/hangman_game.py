...
class HangmanGame:
    """
    Класс для управления игрой. Содержит текущее состояние игры, включая выбранное слово,
    ...
    def __init__(self, word_dictionary: WordDictionary, difficulty: str, category: str,
    ...
    def reset_game(self, difficulty: str, category: str, error_handler: ErrorHandler) -> None:
        """
        ...
        Args:
            ...
            error_handler: Обработчик ошибок.
        """
        word = self.word_dictionary.get_word_by_difficulty_and_category(difficulty, category, error_handler)
        if word is None:
            raise ValueError("No word found for the given difficulty and category")
        self.current_word = word
        self.guessed_letters = set()
        self.remaining_attempts = self.attempts_by_difficulty[self.current_word.difficulty]
    ...
    def guess_letter(self, letter: str) -> None:
        ...
        if letter in self.guessed_letters:
            raise ValueError("Эта буква уже была использована, попробуйте другую")
    ...
    def word_display(self) -> str:
        ...
        """
        return ''.join(letter if letter in self.guessed_letters else '_' for letter in self.current_word.word)
    ...
    def is_game_over(self) -> bool:
        ...
        """
        return self.remaining_attempts <= 0 or self.word_display == self.current_word.word

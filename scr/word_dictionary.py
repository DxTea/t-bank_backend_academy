...
from scr.custom_errors import InvalidParametersError
from scr.list_of_words import words
...
class WordDictionary:
    """
    Класс представляет словарь слов, из которого выбирается случайное слово для игры.
    """
    ...
    def add_word(self, word: str, hint1: str, hint2: str, category: str) -> None:
        """
    ...
    def get_word_by_difficulty_and_category(self, difficulty: str, category: str, error_handler: ErrorHandler) -> \
            Optional[Word]:
        """
        ...
        Returns:
            Word: Слово из словаря, соответствующее заданным сложности и категории.
        """
        filtered_words = [word for word in self.words if
                          (word.difficulty == difficulty or difficulty == "Случайно") and
                          (word.category == category or category == "Случайная")]

        if not filtered_words:
            error_message = (f"Не нашлось слов с выбранной категорией ({category})  "
